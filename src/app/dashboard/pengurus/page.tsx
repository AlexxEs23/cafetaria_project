...
export default function PengurusDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'items' | 'pending' | 'transactions' | 'users'>('pending')
  const [userRole, setUserRole] = useState<'MITRA' | 'KASIR' | 'USER' | 'ALL'>('ALL')
    ...
    const fetchData = async () => {
      ...
      try {
        if (activeTab === 'transactions') {
          ...
          const res = await fetch(`/api/items${status ? `?status=${status}` : ''}`)
          const data = await res.json()
          setItems(data.items || [])
        }
      } catch (error) {
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-4">
                <h2 className="text-2xl font-bold text-gray-900">Kelola Akun Users</h2>
                <select
                  value={userRole}
                  onChange={(e) => setUserRole(e.target.value as 'MITRA' | 'KASIR' | 'USER' | 'ALL')}
                  ...
                  {users.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-6 py-8 text-center text-gray-500">
                        {userRole === 'ALL' ? 'Belum ada user terdaftar' : `Belum ada ${userRole} terdaftar`}
                      </td>
                    </tr>
                  ) : (
                    users.map((user) => (
                      <tr key={user.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                            user.role === 'MITRA' ? 'bg-purple-100 text-purple-800' :
                            user.role === 'KASIR' ? 'bg-green-100 text-green-800' :
                  ...
                  <tr key={transaction.id}>
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">
                      {transaction.details.map((d, i) => (
                        <div key={i}>{d.item.namaBarang} x{d.jumlah}</div>
                      ))}
                    </td>
                  </tr>
                ))}
                ...
                <div className="p-4">
                  ...
                  <div className="space-y-1 text-sm text-gray-600 mb-4">
                    ...
                    <p>Status: <span className={`font-semibold ${
                      ...
                      item.status === 'DITOLAK' ? 'text-red-600' : 'text-gray-600'
                    }`}>{item.status}</span></p>
