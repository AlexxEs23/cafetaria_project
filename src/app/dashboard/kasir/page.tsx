...
export default function KasirDashboard() {
  ...
  const addToCart = useCallback((item: Item) => {
    ...
    setToast({ message: `${item.namaBarang} ditambahkan ke keranjang`, type: 'success' })
  }, [cart])
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-teal-700 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-teal-100">Selamat datang, {session?.user?.name}</p>
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div key={item.id} className="bg-white rounded-lg shadow-md overflow-hidden">
                  <div className="flex">
                    ...
                    <div className="p-4 flex-1">
                      <h3 className="font-semibold text-gray-900 mb-2">{item.namaBarang}</h3>
                      <p className="text-sm text-gray-900 font-medium mb-2">
                        Rp {item.hargaSatuan.toLocaleString('id-ID')}
                      </p>
                      <p className="text-xs text-gray-900 mb-3">Stok: {item.jumlahStok}</p>
                      <button
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow-md p-6 sticky top-4">
              ...
              {cart.length === 0 ? (
                <p className="text-center text-gray-900 py-8">Keranjang kosong</p>
              ) : (
                <>
                    ...
                    {cart.map((cartItem) => (
                      <div key={cartItem.item.id} className="border-b pb-4">
                        <div className="flex justify-between items-start mb-2">
                          <h4 className="font-medium text-sm text-gray-900">{cartItem.item.namaBarang}</h4>
                          <button
                        ...
                        <p className="text-sm text-gray-900 mb-2">
                          Rp {cartItem.item.hargaSatuan.toLocaleString('id-ID')}
                        </p>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="ml-auto text-sm font-medium text-teal-700">
                            Rp {(cartItem.item.hargaSatuan * cartItem.quantity).toLocaleString('id-ID')}
                          </span>
                        </div>
                      </div>
                    ))}
          ...
          <div className="space-y-6">
            <h2 className="text-xl font-bold mb-4">Pesanan Menunggu Persetujuan</h2>
            
            {pendingOrders.length === 0 ? (
              <div className="bg-white rounded-lg shadow-md p-8 text-center">
                <p className="text-gray-500">Tidak ada pesanan yang menunggu persetujuan</p>
              </div>
            ) : (
                  ...
                  <div key={order.id} className="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-gray-900">{order.customerLocation || '-'}</p>
                      </div>
                    </div>
                    ...
                    {order.notes && (
                      <div className="mb-4">
                        ...
                        <p className="text-gray-900 bg-gray-50 p-2 rounded">{order.notes}</p>
                      </div>
                    )}
                        ...
                        {order.details?.map((detail) => (
                          <div key={detail.id} className="flex justify-between text-sm">
                            <span className="text-gray-900">
                              {detail.item?.namaBarang || 'Item'} x {detail.jumlah}
                            </span>
      ...
      <Modal isOpen={showModal} onClose={closeModal} title="Transaksi Berhasil" maxWidth="lg">
        <div className="space-y-4">
          ...
          {transactionResult && (
            <div className="bg-gray-50 rounded-lg p-6 space-y-3">
              <div className="flex justify-between items-center pb-3 border-b">
                ...
                <span className="text-gray-900 font-semibold">#{transactionResult.transaction.id}</span>
              </div>
              
              <div className="space-y-2">
                <h4 className="font-medium text-gray-900 mb-3">Detail Pembelian:</h4>
                {transactionResult.transaction.details?.map((detail: TransactionDetailWithItem, index: number) => {
              ...
              <div className="flex justify-between items-center pt-3 border-t">
                ...
                <span className="text-lg font-bold text-teal-700">
                  Rp {(transactionResult.transaction.totalHarga || 0).toLocaleString('id-ID')}
                </span>
              </div>

              <div className="text-sm text-gray-900 pt-2">
                <p>Tanggal: {transactionResult.transaction.tanggal ? new Date(transactionResult.transaction.tanggal).toLocaleDateString('id-ID', {
                  day: 'numeric',
                  month: 'long',
                  year: 'numeric',
                  hour: '2-digit',
